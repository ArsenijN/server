...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # --- Phase 1: forward FluxDrop API + app shell to CDN HTTP port ---
        ...
        if any(_clean == p.rstrip('/') or _clean.startswith(p) for p in _CDN_PREFIXES_GET):
            ...
            self.send_header('Location', f"http://{_PUBLIC_DOMAIN}:{CDN_HTTP_PORT}{_clean}{_qs}")
            self.send_header('Content-Length', '0')
            self.end_headers()
            return
    ...
    def _proxy_to_cdn(self):
        """Forward this request body to the CDN HTTP port and pipe the response back.

        Returns True if proxied (caller should return), False if not a CDN path.
        """
        ...
        target_url = f"http://{_PUBLIC_DOMAIN}:{CDN_HTTP_PORT}{self.path}"
