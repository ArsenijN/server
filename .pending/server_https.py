...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # --- Phase 1: forward FluxDrop API + app shell to CDN HTTPS port ---
        # /auth/*, /api/*, /fluxdrop_pp/* all live on server_cdn.py.
        # A 308 redirect is used so the browser re-sends POST bodies unchanged.
        # Phase 2 (deprecation) will update script.js to target the standard
        # port directly and remove these blocks.
        _CDN_PREFIXES_GET = ('/auth/', '/api/', '/fluxdrop_pp/')
        _clean = requested_path.split('?')[0]
        if any(_clean == p.rstrip('/') or _clean.startswith(p) for p in _CDN_PREFIXES_GET):
            _qs = ('?' + requested_path.split('?', 1)[1]) if '?' in requested_path else ''
            self.send_response(308)
            self.send_header('Location', f"https://{PUBLIC_DOMAIN}:{CDN_HTTPS_PORT}{_clean}{_qs}")
            self.send_header('Content-Length', '0')
            self.end_headers()
            return
    ...
    def do_POST(self):
        ...
        # Phase 1: proxy /auth/* and /api/* straight to the CDN server.
        if self._proxy_to_cdn():
            return
    ...
    # Forwards the incoming request body to the CDN HTTPS port and streams the
    # response back verbatim.  Used for POST/PATCH/DELETE on /auth/* and /api/*
    # so that fetch() calls from script.js work without CORS pre-flight issues
    # that a plain 308 redirect across ports would trigger.
    def _proxy_to_cdn(self):
        """Forward this request to the CDN HTTPS port and pipe the response back.

        Returns True if the proxy succeeded (caller should return immediately),
        False if the path is not a CDN path (caller should handle normally).
        """
        import urllib.request as _urlreq
        import ssl as _ssl

        _clean = self.path.split('?')[0]
        _CDN_PREFIXES = ('/auth/', '/api/')
        if not any(_clean == p.rstrip('/') or _clean.startswith(p) for p in _CDN_PREFIXES):
            return False

        target_url = f"https://{PUBLIC_DOMAIN}:{CDN_HTTPS_PORT}{self.path}"
        ...
        # Build forwarded headers — pass Authorization and Content-Type through,
        # strip hop-by-hop headers that must not be forwarded.
        _skip = {'host', 'connection', 'transfer-encoding', 'keep-alive',
        ...
        req = _urlreq.Request(target_url, data=body,
                              headers=fwd_headers, method=self.command)

        # CDN uses a self-signed cert — skip verification on the internal hop.
        ctx = _ssl.SSLContext(_ssl.PROTOCOL_TLS_CLIENT)
        ctx.check_hostname = False
        ctx.verify_mode = _ssl.CERT_NONE

        try:
            with _urlreq.urlopen(req, context=ctx, timeout=60) as resp:
                body_out = resp.read()
                self.send_response(resp.status)
                _skip_resp = {'transfer-encoding', 'connection'}
                for hdr, val in resp.headers.items():
                    if hdr.lower() not in _skip_resp:
            ...
            if isinstance(exc, _urlreq.HTTPError):
                body_out = exc.read()
                self.send_response(exc.code)
                for hdr, val in exc.headers.items():
                ...
                print(f"CDN proxy error: {exc}")
                body_out = b'{"error":"CDN proxy error"}'
    ...
    def do_PATCH(self):
        """Forward PATCH /auth/* and /api/* to CDN; everything else is 405."""
        with blacklist_lock:
    ...
    def do_DELETE(self):
        """Forward DELETE /auth/* and /api/* to CDN; everything else is 405."""
        with blacklist_lock:
            if self.client_address[0] in current_blacklist:
                self._set_headers(403, 'text/html')
                self.wfile.write(b"<h1>403 Forbidden</h1>")
