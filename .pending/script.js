        ...
        // --- CONFIGURATION ---
        // ======================================================================
// Phase 2: all API traffic goes through the standard HTTPS port on the
// server_https.py front-end, which proxies /auth/* and /api/* to the CDN
// internally.  The old direct-to-CDN port constants (:63512, :64800) are
// removed.  If you need to roll back to direct CDN access temporarily,
// restore the two commented lines below and revert API_BASE_URL.
//
// const API_HTTPS_LEGACY = `https://${window.location.hostname}:64800`;
// const API_HTTP_LEGACY  = `http://${window.location.hostname}:63512`;
...
async function fetchWithFallback(url, options) {
    // Phase 2: API_BASE_URL is always same-origin, so there is no HTTP/HTTPS
    // fallback to attempt.  This wrapper is kept for call-site compatibility;
    // it now just forwards to fetch() directly.
    // If the body is an ArrayBuffer, snapshot it now in case callers ever add
    // retry logic in the future.
    if (options && options.body instanceof ArrayBuffer) {
        options = { ...options, body: options.body.slice(0) };
    }
